...
# Versione 2.0 - Performance Optimized
# Usa sed invece di awk per migliori performance su file grandi
...
LOG_FILE="${SCRIPT_DIR}/logs/fix_docs_old_v2_${TIMESTAMP}.log"
BACKUP_DIR="${SCRIPT_DIR}/backups/docs_old_v2_${TIMESTAMP}"
...
echo -e "${BOLD}${BLUE}=== 📚 SCRIPT DOCS & OLD V2 - RISOLUZIONE CONFLITTI GIT ===${NC}"
echo -e "${BOLD}${BLUE}=== Versione Ottimizzata con sed e batch processing ===${NC}"
...
# Inizializza il log
{
    echo "=== SCRIPT DOCS & OLD V2 - RISOLUZIONE CONFLITTI GIT ==="
    echo "=== $(date) ==="
    echo "=== Versione Ottimizzata con sed e batch processing ==="
...
# Funzione per risolvere conflitti in file .md usando sed (molto più veloce)
resolve_md_conflicts() {
    ...
    # Usa sed per rimuovere i marcatori di conflitto e tenere la versione dopo =======
    # Questo è molto più efficiente di awk per file grandi
    ...
    # Usa sed per risolvere i conflitti:
    # 1. Rimuovi tutto da <<<<<<< fino a ======= (incluso)
    # 2. Rimuovi le linee >>>>>>> 
    ...
    if [[ -s "$temp_file" ]] && ! grep -q "^<<<<<<< " "$temp_file" 2>/dev/null && ! grep -q "^>>>>>>> " "$temp_file" 2>/dev/null; then
        ...
        echo -e "${RED}❌ Errore dopo risoluzione in: $file${NC}"
        log_with_timestamp "ERRORE_RISOLUZIONE: $file"
        rm -f "$temp_file"
        return 1
    fi
}
...
# Funzione principale
main() {
    echo -e "${BOLD}${PURPLE}🚀 Avvio risoluzione conflitti Git per .md e .old files (V2)...${NC}"
