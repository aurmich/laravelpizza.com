...
# Script ottimizzato per risolvere conflitti Git in file .md e .old
# Versione 3.0 - Robust Conflict Resolution
# Gestisce pattern di conflitto incompleti e edge cases
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --batch-size N     Processa N file alla volta (default: 10)"
...
LOG_FILE="${SCRIPT_DIR}/logs/fix_docs_old_v3_${TIMESTAMP}.log"
BACKUP_DIR="${SCRIPT_DIR}/backups/docs_old_v3_${TIMESTAMP}"
...
echo -e "${BOLD}${BLUE}=== 📚 SCRIPT DOCS & OLD V3 - RISOLUZIONE CONFLITTI GIT ===${NC}"
echo -e "${BOLD}${BLUE}=== Versione Robusta con gestione edge cases ===${NC}"
...
# Inizializza il log
{
    echo "=== SCRIPT DOCS & OLD V3 - RISOLUZIONE CONFLITTI GIT ==="
    echo "=== $(date) ==="
    echo "=== Versione Robusta con gestione edge cases ==="
...
# Funzione migliorata per risolvere conflitti in file .md
resolve_md_conflicts() {
    ...
    # Usa un approccio più robusto con awk che gestisce edge cases
    ...
    # Rileva separatore - da qui in poi teniamo il contenuto
    /^=======/ {
        if (in_conflict) {
            kept_content = true
        }
    ...
    {
        if (!in_conflict) {
            # Fuori dal conflitto, stampa sempre
            print $0
        } else if (kept_content) {
            # Nel conflitto e dopo =======, stampa il contenuto
            print $0
        }
        # Altrimenti siamo nel conflitto prima di ======= - non stampare
    }
    ...
    if [[ -s "$temp_file" ]] && ! grep -q "^<<<<<<< " "$temp_file" 2>/dev/null && ! grep -q "^>>>>>>> " "$temp_file" 2>/dev/null; then
        ...
        echo -e "${RED}❌ Errore dopo risoluzione in: $file (file vuoto o conflitti residui)${NC}"
        log_with_timestamp "ERRORE_RISOLUZIONE: $file - file vuoto o conflitti residui"
        
        # Debug: mostra cosa è successo
        if [ "$VERBOSE" = true ]; then
            echo -e "${RED}Contenuto temp file:${NC}"
            head -10 "$temp_file" 2>/dev/null || echo "File vuoto o illeggibile"
        fi
...
# Funzione principale
main() {
    echo -e "${BOLD}${PURPLE}🚀 Avvio risoluzione conflitti Git per .md e .old files (V3)...${NC}"
