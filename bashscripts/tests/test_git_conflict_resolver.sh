...
assert_no_conflicts() {
    ...
    if ! grep -q "^<<<<<<< HEAD" "$file" 2>/dev/null; then
        ...
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: $message (conflicts found in $file)"
        return 1
    fi
}
...
EOF
    ...
    if gcr_has_conflicts "conflict.txt"; then
        ((TESTS_PASSED++)) || true
        echo "✅ PASS: Detects conflicts correctly"
    else
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: Should detect conflicts"
    fi
    ((TESTS_RUN++)) || true
    ...
    if ! gcr_has_conflicts "no_conflict.txt"; then
        ...
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: Should not detect conflicts"
    fi
    ((TESTS_RUN++)) || true
}
...
test_resolve_incoming() {
    echo ""
    echo "🧪 TEST: Resolve with incoming strategy"
...
test_resolve_head() {
    echo ""
    echo "🧪 TEST: Resolve with head strategy"
...
test_resolve_file_incoming() {
    echo ""
    echo "🧪 TEST: Resolve file with incoming"
...
EOF
    ...
    if gcr_resolve_file "file_test.txt" "incoming" "" "false"; then
        ((TESTS_PASSED++)) || true
        echo "✅ PASS: File resolved successfully"
    else
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: File resolution failed"
    fi
    ((TESTS_RUN++)) || true
    ...
    if grep -q "INCOMING content" "file_test.txt"; then
        ((TESTS_PASSED++)) || true
        echo "✅ PASS: Contains incoming content"
    else
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: Should contain incoming content"
    fi
    ((TESTS_RUN++)) || true
}
...
test_binary_detection() {
    ...
    if gcr_is_binary "binary_file.bin"; then
        ((TESTS_PASSED++)) || true
        echo "✅ PASS: Detects binary files"
    else
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: Should detect binary"
    fi
    ((TESTS_RUN++)) || true
}
...
EOF
    ...
    if echo "$result" | grep -q '"has_conflicts": true'; then
        ((TESTS_PASSED++)) || true
        echo "✅ PASS: Analysis detects conflicts"
    else
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: Analysis should detect conflicts"
    fi
    ((TESTS_RUN++)) || true
}
...
test_multiple_conflicts_in_file() {
    echo ""
    echo "🧪 TEST: Multiple conflicts in single file"
...
EOF
    ...
    if grep -q "First INCOMING" "multi_conflict.txt" && \
       grep -q "Second INCOMING" "multi_conflict.txt"; then
        ((TESTS_PASSED++)) || true
        echo "✅ PASS: All conflicts resolved with incoming"
    else
        ((TESTS_FAILED++)) || true
        echo "❌ FAIL: Not all conflicts resolved correctly"
    fi
    ((TESTS_RUN++)) || true
}
