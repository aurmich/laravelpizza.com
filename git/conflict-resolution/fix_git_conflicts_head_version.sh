...
# Script avanzato per risolvere automaticamente i conflitti Git scegliendo sempre la "HEAD version" (current/ours)
# ATTENZIONE: Questo script mantiene la versione HEAD (prima di =======), NON INCOMING (dopo =======)
# Autore: AI Assistant con poteri della supermucca
# Versione: 5.0 - HEAD Version Edition
# 
# Questo script risolve tutti i conflitti Git scegliendo sempre la versione "HEAD" (current/ours)
# (contenuto tra <<<<<<< HEAD e =======) invece della versione INCOMING (tra ======= e >>>>>>>)
#
# USAGE:
#   ./fix_git_conflicts_head_version.sh [DIRECTORY] [--dry-run] [--no-confirm] [--help]
#
# ESEMPI:
#   # Dry-run nella directory corrente
#   DRY_RUN=true ./fix_git_conflicts_head_version.sh
#
#   # Risolvi conflitti in una directory specifica
#   ./fix_git_conflicts_head_version.sh /path/to/repo
#
#   # Risolvi senza conferma interattiva
#   ./fix_git_conflicts_head_version.sh --no-confirm
...
if [ "$SHOW_HELP" = true ]; then
    cat <<EOF
${BOLD}${BLUE}Script Risoluzione Conflitti Git (HEAD Version)${NC}

${BOLD}ATTENZIONE:${NC} Questo script mantiene la versione ${GREEN}HEAD${NC} (prima di =======), 
${GREEN}NON${NC} la versione INCOMING (dopo di =======)!

${BOLD}USAGE:${NC}
    ./fix_git_conflicts_head_version.sh [DIRECTORY] [OPTIONS]
...
${BOLD}ESEMPI:${NC}
    ...
    # Risolvi conflitti in una directory specifica
    ./fix_git_conflicts_head_version.sh /var/www/_bases/base_techplanner_fila4_mono
...
LOG_FILE="${LOG_DIR}/fix_conflicts_head_$(date +%Y%m%d_%H%M%S).log"

# Banner
echo -e "${BOLD}${BLUE}=== 🐄 SCRIPT SUPERMUCCA V5.0 - RISOLUZIONE CONFLITTI GIT (HEAD) ===${NC}"
echo -e "${GREEN}✅ ATTENZIONE: Questo script mantiene la versione ${BOLD}HEAD${NC}${GREEN} (prima di =======)${NC}"
echo -e "${GREEN}✅             ${BOLD}NON${NC}${GREEN} la versione INCOMING (dopo di =======)${NC}"
...
log "START head-version resolver (dry=$DRY_RUN, dir=$BASE_DIR)"

# Resolver AWK: tiene solo la sezione HEAD (prima di =======)
# ATTENZIONE: Questo mantiene HEAD, non INCOMING!
resolve_with_head() {
  awk '
